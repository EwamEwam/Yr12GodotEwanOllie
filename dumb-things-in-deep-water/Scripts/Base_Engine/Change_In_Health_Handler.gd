...
##[br]FORCED_DAMAGE: Forces a change in hp no matter current modifers and status effects
...
enum TYPES {GENERAL_DAMAGE,GENERAL_HEAL,MAGIC_DAMAGE,FALL_DAMAGE,FORCED, INCREMENTAL_PIERCE,INCREMENTAL_NO_PIERCE}

##Determines what damage types get Player Model and HUD feedback
const SHOWS_PARTICLES = [TYPES.GENERAL_DAMAGE,TYPES.MAGIC_DAMAGE,TYPES.GENERAL_HEAL,TYPES.FALL_DAMAGE,TYPES.FORCED]
	...
	if object != Playerstats.player:
		...
		match type:
			TYPES.GENERAL_DAMAGE:
				...
				if amt >= 1:
					amt = max(1,amt - Playerstats.defense)
